...
<h6>Add new category for store products </h6><br/>

<?php 
if(isset($info)){echo '<p>'.$info.'</p>';}
$attributes = array('class' => 'store_cat_form');
echo form_open('store/category', $attributes);
?>
    ...
    <div class"col-12">
        ...
        <h4>All Categories </h4><br/>
                <?php
                echo form_open('store/delete_category/');
                ?>
                    ...
                    <tbody>
                        ...
                        if($categories != false)
                        {
                            foreach($categories as $row)
                            {
                                ...
                                echo '<td><a href="'.base_url.'index.php/store/edit_category/'.$row['id'].'">'.$row['cat_name'].'</a></td>';
                                echo '<td>'.$row['cat_desc'].'</td>';
                                echo '<td>'.$row['date_added'].'</td>';
                                echo '<td><a href="'.base_url.'index.php/store/edit_category/'.$row['id'].'"><input type="button" class="btn btn-sm btn-link" value="Edit"></a></td>';
                                echo '<td><input url="'.base_url.'index.php/store/delete_category/'.$row['id'].'" id="delete" type="button" class=" delete_btn btn btn-sm btn-link text-danger" value="Delete"></td>';
                                echo '</tr>';
                            }
                        }
                        else
